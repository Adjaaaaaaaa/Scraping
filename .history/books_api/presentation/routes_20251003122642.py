...
def search_books(
    ...
    rating: Optional[int] = Query(None, ge=1, le=5),  # ← nouveau paramètre
    skip: int = 0,
    limit: int = 100,
    repo: BookRepository = Depends(get_book_repo)
):
    return repo.search(category, min_price, max_price, availability.value if availability else None, skip, limit)
